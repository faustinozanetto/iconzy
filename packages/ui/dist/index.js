...
var RangeInput = import_react6.default.forwardRef((props, ref) => {
  const _a = props,
    { id = 'default-id', label, onValueChanged } = _a,
    rest = __objRest(_a, ['id', 'label', 'onValueChanged']);
  const [value, setValue] = (0, import_react6.useState)(Number(rest.placeholder) || 0);
  const debouncedValue = use_debounce_default(value, 100);
  const handleChange = (event) => {
    const { value: value2 } = event.target;
    setValue(value2);
...
var ColorInput = import_react7.default.forwardRef((props, ref) => {
  const _a = props,
    { id = 'default-id', label, onValueChanged } = _a,
    rest = __objRest(_a, ['id', 'label', 'onValueChanged']);
  const [value, setValue] = (0, import_react7.useState)(rest.placeholder || '');
  const debouncedValue = use_debounce_default(value, 100);
  const handleChange = (event) => {
    const { value: value2 } = event.target;
    setValue(value2);
